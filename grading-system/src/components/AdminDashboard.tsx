...
const AdminDashboard = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedCourse) {
      ...
      try {
        await axios.put(
          `http://localhost:4000/admin/course/${selectedCourse.id}`,
          form,
          { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
        );
        ...
        const updatedCourses = await axios.get("http://localhost:4000/admin/courses", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
      ...
      try {
        await axios.post(
          "http://localhost:4000/admin/course",
          form,
          { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
        );
  ...
  const handleCourseDelete = async (courseId: number) => {
    if (window.confirm("Are you sure you want to delete this course?")) {
      try {
        await axios.delete(`http://localhost:4000/admin/course/${courseId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
