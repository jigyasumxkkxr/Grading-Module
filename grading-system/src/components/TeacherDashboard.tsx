...
const TeacherDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get("http://localhost:4000/teacher/courses", {
  ...
  const handleCourseClick = async (courseId: number) => {
    try {
      const response = await axios.get(
        `http://localhost:4000/teacher/course/${courseId}/students`,
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
  ...
  const handleAssignGrade = async () => {
    if (selectedStudent && selectedCourse) {
      ...
      try {
        ...
        await axios.post(
          `http://localhost:4000/teacher/course/${selectedCourse.id}/student/${selectedStudent.id}/grade`,
